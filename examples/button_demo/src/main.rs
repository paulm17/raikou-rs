...
//!
//! Shows one button per variant, all reporting to a shared counter label.
//! A trimmed version of the app's tool-loop bootstrap: fyrox engine with a
//! single `UserInterface` in `RenderMode::OnChanges`, a `ControlFlow::Wait`
//! loop and a message poll that dispatches into the raikou registry.
