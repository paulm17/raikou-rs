...
use raikou_layout::{
    HorizontalAlignment, LayoutContext, LayoutElement, SizedBox, VerticalAlignment, arrange_element,
    measure_element,
};
...
fn arrange_core_aligns_with_margin_and_stretch_rules() {
    ...
    measure_element(&mut node, &mut ctx, Size::new(100.0, 100.0));
    arrange_element(&mut node, &mut ctx, Rect::from_xywh(0.0, 0.0, 100.0, 100.0));
...
fn arrange_core_stretch_fills_slot_when_no_explicit_size_exists() {
    ...
    measure_element(&mut node, &mut ctx, Size::new(100.0, 80.0));
    arrange_element(&mut node, &mut ctx, Rect::from_xywh(0.0, 0.0, 100.0, 80.0));
...
fn arrange_core_respects_left_right_top_and_center_alignments() {
    ...
    measure_element(&mut left_top, &mut ctx, Size::new(100.0, 80.0));
    arrange_element(&mut left_top, &mut ctx, Rect::from_xywh(0.0, 0.0, 100.0, 80.0));
    assert_eq!(
    ...
    measure_element(&mut right_center, &mut ctx, Size::new(100.0, 80.0));
    arrange_element(&mut right_center, &mut ctx, Rect::from_xywh(0.0, 0.0, 100.0, 80.0));
    assert_eq!(
