...
use raikou_layout::{
    LayoutContext, LayoutElement, OverlayLayer, SizedBox, StackPanel, arrange_element, measure_element,
};
...
fn avalonia_overlay_layer_measures_to_root_size_and_paints_in_overlay_layer() {
    ...
    assert_eq!(
        measure_element(&mut overlay, &mut ctx, Size::new(200.0, 120.0)),
        Size::new(200.0, 120.0)
    );
    arrange_element(&mut overlay, &mut ctx, Rect::from_xywh(0.0, 0.0, 200.0, 120.0));
...
fn avalonia_overlay_layer_remains_stable_during_resize() {
    ...
    measure_element(&mut overlay, &mut ctx, Size::new(100.0, 80.0));
    arrange_element(&mut overlay, &mut ctx, Rect::from_xywh(0.0, 0.0, 100.0, 80.0));
    assert_eq!(overlay.available_size(), Size::new(100.0, 80.0));

    measure_element(&mut overlay, &mut ctx, Size::new(160.0, 90.0));
    arrange_element(&mut overlay, &mut ctx, Rect::from_xywh(0.0, 0.0, 160.0, 90.0));
...
fn avalonia_overlay_children_do_not_affect_normal_content_desired_size() {
    let mut font_system = raikou_layout::FontSystem::new();
    let mut ctx = LayoutContext::new(&mut font_system);
    let mut content = StackPanel::new();
    ...
    assert_eq!(
        measure_element(&mut content, &mut ctx, Size::new(100.0, 80.0)),
        Size::new(40.0, 20.0)
    );
    assert_eq!(
        measure_element(&mut overlay, &mut ctx, Size::new(100.0, 80.0)),
